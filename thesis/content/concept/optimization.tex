...
\%todo{Why do we need to apply to translated code and not intermediate? most optimizations typically on intermediate} 
...
The circuit graph is a graphical description of a quantum circuit; it is an acyclic and directed graph. For each qubit in the circuit, there exists both an input node and an output node. Furthermore, each input node has exactly one outgoing vertex while each output node has exactly one incoming vertex. Besides the input and output nodes, all other nodes represent gates in the circuit. For all gate nodes, the number of incoming vertices is equivalent to the number of outgoing vertices. Additionally, the number of incoming, or outgoing, vertices is the same as the number of arguments for the gate. Importantly, in this case, any qubits controlling the application of the gate, \eg the first qubit in a controlled-not gate, also count to the number of arguments. In turn, there exists a vertex pair of an incoming and outgoing vertex each that represent a qubit to which the gate is applied. Therefore, for each qubit, there exists one path from its input node to its output node such that all gates that are applied to it are visited in order of application.

% Formal Definition
Extension of graph definition, includes set of qubits $Q$ associated with graph and relations between input, output nodes and qubit $Q_V$ as well as the edges and qubits $Q_E$. 
\begin{align*}
    C &= (V, E, Q, Q_E, Q_V)\\
...
\begin{algorithm}
    ...
    \While{$repeat = true$}{
        ...
        {
            $start \gets i \quad \text{where } (i, q) \in Q_V \land i \in I$\;
            ...
            $last \gets start$\;
            \For(\Comment*[f]{Iterate paths up to $max$ rule length}){$i \gets 1$ \KwTo $max$}{
